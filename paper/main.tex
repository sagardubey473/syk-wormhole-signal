...
\title{Signal-chaos decoupling in the SYK traversable wormhole}
...
to substantiate claims of holographic behavior.
\end{abstract}
...
The signal-chaos decoupling has a practical consequence for quantum
...
Since the signal is invariant down to $p=0.02$, this allows a
substantial reduction. At $N=10$: the dense Hamiltonian requires
${\sim}1{,}260$ controlled-NOT (CNOT) gates per Trotter step, while $p=0.02$ requires
${\sim}24$---a $52\times$ reduction. We note, however, that the variance
rescaling [Eq.~\eqref{eq:sparse_variance}] increases the spectral norm
of individual coupling terms by $1/\sqrt{p}$, which may require smaller
Trotter steps to maintain a fixed simulation error, partially offsetting
the per-step savings. The net gate reduction after accounting for
Trotterization error bounds remains substantial but is smaller than the
naive $52\times$ estimate.
...
quantum chaos, and that future quantum gravity simulation experiments
should supplement the transmission signal with independent chaos diagnostics
to substantiate claims of holographic behavior.

\begin{acknowledgments}
The author thanks the Stony Brook University Department of Physics and
Astronomy for computational resources.
\end{acknowledgments}

\vspace{1em}
\noindent\textit{Data and code availability.}---All numerical data, analysis
code, and figure-generation scripts are available at \url{https://github.com/sagardubey473/syk-wormhole-signal-chaos}.
